...
class DiscoveryService : Disposable {
    ...
    fun createDiscoveryFile(port: Int) {
        try {
            ...
            val openCodeIdeDir = Path.of(tmpDir, "opencode", "ide")

            if (!Files.exists(openCodeIdeDir)) {
                Files.createDirectories(openCodeIdeDir)
            }

            val fileName = "opencode-ide-server-$pid-$port.json"
            val file = openCodeIdeDir.resolve(fileName).toFile()
