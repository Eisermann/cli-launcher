...
class OpenCodeTerminalManager(private val project: Project) {
    ...
    companion object {
        private val OPENCODE_TERMINAL_KEY = Key.create<Boolean>("opencode.link.openCodeTerminal")
        private val OPENCODE_TERMINAL_RUNNING_KEY = Key.create<Boolean>("opencode.link.openCodeTerminal.running")
        private val OPENCODE_TERMINAL_CALLBACK_KEY = Key.create<Boolean>("opencode.link.openCodeTerminal.callbackRegistered")
        private val OPENCODE_TERMINAL_SHIFT_ENTER_KEY = Key.create<Boolean>("opencode.link.openCodeTerminal.shiftEnterRegistered")
    }
    ...
    private val scriptFactory = CommandScriptFactory(
        ...
        supportsPosixShell = {
            if (!SystemInfoRt.isWindows) {
                ...
                runCatching { service<OpenCodeLinkSettings>().state.winShell == WinShell.WSL }.getOrDefault(false)
            }
        }
    )
