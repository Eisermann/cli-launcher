package com.github.eisermann.opencodelauncher.cli
...
/**
 * Utility object for building command-line arguments for OpenCode execution.
 *
 * This builder translates the plugin's settings into appropriate command-line arguments
 * that can be passed to the OpenCode CLI tool. It handles:
...
object OpenCodeArgsBuilder {
    ...
    /**
     * Builds the command-line argument list for OpenCode based on the provided settings state.
     *
     ...
     * @return A list of command-line arguments to pass to OpenCode
     *
     * @example
     * For settings with mode=FULL_AUTO and model=GPT_5_1_CODEX:
     * Returns: ["--full-auto", "--model", "gpt-5.1-codex"]
     */
    fun build(
        state: OpenCodeLauncherSettings.State,
        port: Int? = null,
        ...
        val modelName: String? = when (state.model) {
            Model.DEFAULT -> null // Use CLI default model
        ...
        val reasoningEffort: String? = when (state.modelReasoningEffort) {
            ModelReasoningEffort.DEFAULT -> null // Use CLI default
